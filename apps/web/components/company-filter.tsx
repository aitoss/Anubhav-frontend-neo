...
export function CompanyFilterList({ activeCompany, onSelect }: CompanyFilterProps) {
    ...
    <div className="flex flex-col gap-1">
      {companies.map((item) => (
        <button
          ...
          className={cn(
            "hover:bg-accent flex cursor-pointer items-center justify-between rounded-md px-2 py-1.5 text-left transition-colors",
...
export function CompanyFilter({ activeCompany, onSelect }: CompanyFilterProps) {
  return (
    <aside className="w-full">
      <h5 className="bg-background sticky top-0 z-10 mb-2 pb-2 text-xl font-medium">
        Filter by Company
      </h5>
      <CompanyFilterList activeCompany={activeCompany} onSelect={onSelect} />
    </aside>
  )
}
